// Global Config
const moneyThresholdPercentage = 0.9;
...
function runScript(ns, scriptName, target, threads) {
  ...
  let usableServers = allServers.filter((p) => ns.hasRootAccess(p) && ns.getServerMaxRam(p) > 0);
  ...
  for (const server of usableServers) {
    ...
    ns.print(`Starting script ${scriptName} on ${server} with ${possibleThreads} threads.`);
