...
export async function main(ns) {
  ...
  let numOfBatches = Math.ceil(
    (ns.getScriptRam('/deploy/weaken.js') * callsToFinalMinSec) /
      (ns.getServerMaxRam('home') - (ns.getServerUsedRam('home') + ns.getScriptRam('/prog/prep.js', 'home'))),
  );
  ns.tprint(
    `[${grgNow()}] - Trigger final weaken script with ${Math.max(
      callsToFinalMinSec,
      1,
    )} threads over ${numOfBatches} batches.`,
  );
  for (let i = 0; i < numOfBatches; i++) {
    ns.tprint(
      `DEBUG - Attempting to launch final weaken with ${Math.max(
        Math.floor(callsToFinalMinSec / numOfBatches),
      )} threads in batch number ${i + 1} with duration of ${ns.nFormat(
        ns.getWeakenTime(serverName) / 1000,
        '0,0[.]00',
      )} seconds.`,
    );
    let returnCode = ns.exec(
      '/deploy/weaken.js',
      'home',
      Math.max(Math.floor(callsToFinalMinSec / numOfBatches), 1),
      serverName,
    );
    if (returnCode === 0) {
...
async function execBatch(ns, hostName, scriptPath, waitTime, targetServer) {
  ...
  if (returnCode === 0) {
    grgLog(
      ns,
      `Not enough memory to run a batch for script ${scriptPath} on host ${hostName} against ${targetServer} with ${batchThreads} threads using ${
        scriptRAM * batchThreads
      } GB of RAM.`,
    );
    return false;
    ...
    grgLog(
      ns,
      `Skipping attempt at zero thread batch for script ${scriptPath} on host ${hostName} against ${targetServer} with ${batchThreads} threads using ${
        scriptRAM * batchThreads
      } GB of RAM.`,
    );
    ...
    grgLog(
      ns,
      `Running a batch for script ${scriptPath} on host ${hostName} against ${targetServer} with duration ${ns.nFormat(
        waitTime / 1000,
        '0,0[.]00',
      )} seconds with ${batchThreads} threads using ${scriptRAM * batchThreads} GB of RAM.`,
    );
    await ns.sleep(waitTime);
    return true;
  }
