...
export async function main(ns) {
  ...
  if (cmd === 'full') {
    ...
    for (const serverName of serverCandidates) {
      ...
      ns.tprint(
        `Name: ${serverName}, \t\t\tMax:${ns.nFormat(maxMoney, '$0,0[.]00')}, \t\t\tScore: ${ns.nFormat(
          score,
          '0,0[.]00',
        )}`,
      );
...
function getBestMoneyServer(ns) {
  ...
  let serverShortList = serverList.filter(
    (x) => ns.getServerRequiredHackingLevel(x) <= Math.floor(ns.getHackingLevel() / 2),
  );
  serverShortList = serverShortList.filter((x) => ns.getServerMaxMoney(x) !== 0);
  serverShortList.sort(
    (a, b) =>
...
function getFullNetworkInner(ns, serverName, path, visited) {
  ...
  for (const child of children) {
    if (!Object.hasOwnProperty.call(visited, child)) {
