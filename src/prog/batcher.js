...
export async function main(ns) {
  ...
  const hackThreadsPerBatch = Math.max(
    Math.floor(ns.hackAnalyzeThreads(serverName, serverMaxMoney * targetHackPercentage)),
    1,
  );
  ...
  const growThreadsPerBatch = Math.ceil(
    ns.growthAnalyze(serverName, serverMaxMoney / (serverMaxMoney * (1 - targetHackPercentage))),
  );
  ...
  grgLog(
    ns,
    `Total Batch RAM: ${
      hackThreadsTotalRam + growThreadsTotalRam + firstWeakenThreadsTotalRam + secondWeakenThreadsTotalRam
    } GB`,
  );
  ...
  grgLog(
    ns,
    `Total Time For Batch Execution: ${ns.nFormat((timeToWeaken + callBuffer * 4) / 1000, '0,0[.]00')} seconds`,
  );
  // TODO
  return 0;
}
