...
export async function main(ns) {
  ...
  const dontKillList = ['/prog/batcher.js', '/prog/prep.js', '/prog/coffeeparty.js'];
  for (const server of myServers) {
    ...
    if (targetOverride === 'share') {
      ...
      const threadCount = Math.floor(
        (ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / ns.getScriptRam(shareScriptPath),
      );
      if (threadCount === 0) {
        continue;
      }
      ns.tprint(
        'New running script PID on ' +
          server +
          '? (PID 0 means exec failed.): ' +
          ns.exec(shareScriptPath, server, threadCount),
      );
      ...
      const threadCount = Math.floor(
        (ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / ns.getScriptRam(moneyScriptPath),
      );
      if (threadCount === 0) {
        continue;
      }
      ns.tprint(
        'New running script PID on ' +
          server +
          '? (PID 0 means exec failed.): ' +
          ns.exec(moneyScriptPath, server, threadCount, richestServer),
      );
      ns.tprint('Deploy for server ' + server + ' complete!');
    }
  }
